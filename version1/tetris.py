...
class Tetris:
	...
	def __init__(self):
		for i in self.textures:
			self.surfaces.append(pygame.image.load('data/'+i))
		

		self.init_tetris_grid()
		
		self.pick_next_shape()
		pygame.init()
		self.w=pygame.display.set_mode((self.win_w, self.win_h))
		pygame.display.set_caption('Tetris by Shumon Zaman')
		...
		#initialize sound
		pygame.mixer.init()	
		self.theme_snd = pygame.mixer.Sound("data/theme.ogg")
		self.effect_snd.append(pygame.mixer.Sound("data/effect1.ogg"))
		self.effect_snd.append(pygame.mixer.Sound("data/effect2.ogg"))
		self.effect_snd.append(pygame.mixer.Sound("data/effect3.ogg"))
	...
	def check_game_over(self):
		if len(self.tetris_grid) > self.win_h / self.block_size:
			game_over_str = 'Game Over' 
			t=self.font.render(game_over_str, True, (255, 255, 255))
			self.w.blit(t, ((self.win_w/2)-50, self.win_h/2))
			pygame.display.update()
			pygame.time.wait(2000)
			del self.tetris_grid[:]
			del self.texture_grid[:]	
			self.init_tetris_grid()
			self.score = 0
			self.level = 1
			#sys.exit(0)
	...
	def pick_next_shape(self): #randomly
		...
		del self.shape[:]
		path = os.getcwd()+'/data/'+'shapes.txt'
		f = open (path,'r')
		lines = f.readlines()
	...
	def update(self):
		...
		while True:
			...
			self.drop()
			self.w.fill((0, 0, 0))
			self.w.blit(self.background,(0,0))		
